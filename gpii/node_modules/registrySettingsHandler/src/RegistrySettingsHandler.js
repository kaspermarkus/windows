...
windows.registryGetHandler = function (payload) {
    var baseKey = payload.options.hKey;
    var path = payload.options.path;
    var results = fluid.each(payload.settings, function (value, key) {        
        return windows.readRegistryKey(baseKey, path, key, value.dataType);
    });
    return results;
};

windows.registrySettingsHandler.get = function (payload) {
    return gpii.invokeSettingsHandler(payload, windows.registryGetHandler);
};
