...
windows.registrySettingsHandler = fluid.registerNamespace("gpii.windows.registrySettingsHandler");
...
gpii.invokeSettingsHandler = function (payload, handler) {
    ...
    var tmp = fluid.transform(payload, function (oneSolution) {
...
windows.registrySettingsHandler.set = function (payload) {
    console.log("Registry SET returning: ");
...
windows.registrySettingsHandler.get = function (payload) {
    console.log("Registry SET returning:");
    return gpii.invokeSettingsHandler(payload, function(load) {
        var baseKey = load.options.hKey;
        var path = load.options.path;
        var results = fluid.transform(load.settings, function(value, key) {
            var readValue = windows.readRegistryKey(baseKey, path, key, value.dataType);
            // console.log("REGISTRY KEY READ ("+key+") with dataType "+value.dataType+": "+JSON.stringify(readValue));
            return readValue;
        });
        //console.log("Registry GET returning: ", results);
        return results;
    });
};
