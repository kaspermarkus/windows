...
var integrationTestsJSON = {
    "sammy": {
        environments: {
            "windows": [
                {
                    "type": "gpii.windows.registrySettingsHandler.get", 
                    "data": [
                        {
                            "options": {
    ...
    */
    var getOriginalValues = function (fulljson) {
        ...
        fluid.each(origValues, function (json, token) {
            origValues[token].environments.windows = fluid.transform(origValues[token].environments.windows, function (testBlock, textIndex) {
                var args = {};
    ...
    var addTests = function () {
        ...
        fluid.each(integrationTestsJSON, function (json, token) {
            integrationTester.asyncTest("Test "+token+" Login", function () {
                http.get({
                    ...
                    response.on("end", function() {
                        ...
                        fluid.each(json.environments.windows, function (testBlock, textIndex) {
                            var args = {};
                            args[token] = testBlock.data;
                            //wait one second to ensure that the settings have propagated
                            setTimeout(function() {
                                fluid.log("FOR LOGIN: "+JSON.stringify(testBlock));
                                //call the settingshandler to get the settings
                                var changedSettings = fluid.invokeGlobalFunction(testBlock.type, [args]);
                                console.log("Returned from get: "+JSON.stringify(changedSettings));
                                ...
                                fluid.each(changedSettings[token], function (arrayEntry, arrayInd) {
                                    ...
                                    fluid.each(arrayEntry.settings, function (settingValue, settingKey) {
                                        ...
                                        if (settingValue != null && typeof settingValue === 'object' //if object look at value variable
                                            && expectedValue.value != null && settingValue != null) {
                                            jqUnit.assertEquals("Check value variable of setting "+settingKey, expectedValue.value, settingValue.value);
                                        } else {
            ...
            integrationTester.asyncTest("Test "+token+" logout", function () {
                ...
                //jqUnit.assertEquals("Check setting "+settingKey, 1, 1);
                http.get({
                    host: "localhost",
                    port: 8081,
                    ...
                    response.on("end", function() {
                        ...
                        fluid.each(originalValues[token].environments.windows, function (testBlock, textIndex) {
                            var args = {};
                            args[token] = testBlock.data;
                            //wait one second to ensure that the settings have propagated
                            setTimeout(function() {
                                console.log("HELLO WORLD!");
                                fluid.log("FOR LOGOUT: "+JSON.stringify(testBlock));
                                // //call the settingshandler to get the settings
                                // var changedSettings = fluid.invokeGlobalFunction(testBlock.type, [args]);
